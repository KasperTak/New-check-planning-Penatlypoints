    ...
    if circulation_file:
        if connexxion_file:
            with st.spinner("Please wait..."):
                ...
                penalty_mat = 0
                index_mat=[]
                Minuten_materiaalrit=[]
                for j in range(len(data_planning)):
                    if activiteit[j]=='materiaal rit':
                        a = str(data_planning['eindtijd'][j])
                        b = str(data_planning['starttijd'][j])
                        t1 = datetime.strptime(a, '%H:%M:%S')
                        t2 = datetime.strptime(b,'%H:%M:%S')
                        delta_t=str(t1-t2)
                        if delta_t[3] != 'd':
                            ...
                            minuten = int(delta_t[10:12]) + 60*int(delta_t[8])
                            penalty_mat+=minuten
                        Minuten_materiaalrit.append(minuten)
                        index_mat.append(j)
                ...
                for h in range(len(data)):
                    ...
                    lijn = data['buslijn'][h]
                    while not indeomloop == False and ind <= len(data_planning)-1:
